...
public class TlvDataObjectContainer extends TlvValue implements Iso7816, TlvDataStructure {
	...
	public byte[] toByteArray() {
		...
		for(TlvDataObject tlvObject : this.tlvObjects) {
			try {
				...
				ApiLoggerResolver.logError(TlvDataObjectContainer.class.getSimpleName(), e.getMessage());
			}
		}
	...
	public void removeTlvDataObject(TlvTagIdentifier tagIdentifier) {
		...
		final Iterator<TlvDataObject> each = tlvObjects.iterator();
		while (each.hasNext()) {
			if (objToRemove.equals(each.next())) {
				each.remove();
			}
		}
	}
