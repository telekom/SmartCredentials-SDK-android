...
public class WisprOnlineFonStrategy extends WisprOnlineStrategy {

    public WisprOnlineFonStrategy(WisprRedirectCallback redirectCallback) {
    ...
    public WisprRedirectReply checkOnline(WisprService wisprService, String url) throws IOException, WisprException {
        WisprRedirectReply redirectReply = new WisprRedirectParser(mRedirectCallback).parse(wisprService.checkOnline(url).execute().body());
        wisprService.followRedirect(url).execute();
