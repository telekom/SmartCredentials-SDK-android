...
public class WisprOnlineEapStrategy extends WisprOnlineStrategy {

    public WisprOnlineEapStrategy(WisprRedirectCallback redirectCallback) {
    ...
    public WisprRedirectReply checkOnline(WisprService wisprService, String url) throws IOException, WisprException {
        return new WisprRedirectParser(mRedirectCallback).parse(wisprService.checkOnline(url).execute().body());
    }
}
