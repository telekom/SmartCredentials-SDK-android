...
public class WisprAuthenticationEapParser {

    private WisprAuthenticationCallback mAuthenticationCallback;

    public WisprAuthenticationEapParser(WisprAuthenticationCallback authenticationCallback) {
        mAuthenticationCallback = authenticationCallback;
    }

    public SmartWisprAuthenticationEapReply parse(String body) throws WisprException {
        ...
        if (prefixIndex < 0 || postfixIndex < 0 || prefixIndex > postfixIndex) {
            throw new WisprException("Unexpected WISPr EAP response format.");
        }
        ...
        SmartWisprAuthenticationEapReply wispreapReply;

        try {
            wispreapReply = serializer.read(SmartWisprAuthenticationEapReply.class, xml, false);
        } catch (Exception e) {
            throw new WisprException("WISPr EAP response parsing exception.");
        }

        if (wispreapReply == null) {
            throw new WisprException("Could not deserialize WISPr EAP response.");
        } else {
            return wispreapReply;
        }
    }
}
