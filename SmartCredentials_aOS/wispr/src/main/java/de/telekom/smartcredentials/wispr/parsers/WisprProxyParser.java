...
public class WisprProxyParser {

    private WisprRedirectCallback mRedirectCallback;

    public WisprProxyParser(WisprRedirectCallback redirectCallback) {
        mRedirectCallback = redirectCallback;
    }

    public WisprProxyReply parse(String body) throws WisprException {
        ...
        if (prefixIndex < 0 || postfixIndex < 0 || prefixIndex > postfixIndex) {
            throw new WisprException("Unexpected WISPr proxy response format.");
        }
        ...
        WisprProxyReply wisprProxyReply;

        try {
            wisprProxyReply = serializer.read(WisprProxyReply.class, xml, false);
        } catch (Exception e) {
            throw new WisprException("WISPr proxy response parsing exception.");
        }

        if (wisprProxyReply == null) {
            throw new WisprException("Could not deserialize WISPr proxy response.");
        } else {
