...
public class WisprPollParser {

    public WisprPollReply parse(String body) throws WisprException {
        ...
        if (prefixIndex < 0 || postfixIndex < 0 || prefixIndex > postfixIndex) {
            throw new WisprException("Unexpected WISPr poll response format.");
        }
        String xml = body.substring(prefixIndex, postfixIndex + WISPR_RESPONSE_POSTFIX.length());
        final Serializer serializer = new Persister();
        WisprPollReply wisprPollReply;

        try {
            wisprPollReply = serializer.read(WisprPollReply.class, xml, false);
        } catch (Exception e) {
            throw new WisprException("WISPr poll response parsing exception.");
        }

        if (wisprPollReply == null) {
            throw new WisprException("Could not deserialize WISPr poll response.");
        } else {
